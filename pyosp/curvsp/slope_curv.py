...
class Slope_curv(Base_curv):
    ...
    :type width: float
    :param min_slope: minimal slope threshold, defaults to 0
    :type min_slope: float, optional
    :param max_slope: maximal slope threshold, defaults to 90
    :type max_slope: float, optional
    ...
    def __init__(
        ...
        super(Slope_curv, self).__init__(
    ...
    def _swath_left(self, p1, p2, endPoint=False):
        ...
        rasterVal = Geo_slope(p_m, self.raster, self.cell_size).value
        if not (
            (self.rasterXmin <= p_m[0] <= self.rasterXmax)
            and (self.rasterYmin <= p_m[1] <= self.rasterYmax)
            and (self.min_slope <= rasterVal <= self.max_slope)
        ):
            transect_temp = []
        else:
            transect_temp = [p_m]
