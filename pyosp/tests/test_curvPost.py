...
class TestPost:
    ...
    def test_profile_plot(self, orig_homo):
        start_end = os.path.join(dat, 'homo_start_end.shp')
        orig = orig_homo()
        orig.profile_plot(start=0, end=100, color="maroon",
                          points=start_end, s=5, marker="s")
        assert True
    ...
    def test_density_scatter(self, orig_homo):
        ...
        coords = pyosp.point_coords(start_end)
        orig = orig_homo()
        orig.density_scatter(bins=20, start=coords[1], end=coords[0], 
                             cmap='jet', s=1)
        assert True
    ...
    def test_cross_dat(self, orig_homo):
        ...
        assert all([ a==b for a, b in zip(cross_dat["distance"], data_valid["distance"])])
        assert all([a == b for a, b in zip([np.nanmean(p) for p in cross_dat["cross_matrix"]],
    ...
    def test_post_tpi(self, orig_homo):
        ...
        coords = pyosp.point_coords(start_end)
        orig = orig_homo()
        post_dat = orig.post_tpi(radius=50, min_val=0, max_val=100, start=coords[1], end=coords[0],
                                 cross=True)
    ...
    def test_post_slope(self, orig_homo):
        ...
        coords = pyosp.point_coords(start_end)
        orig = orig_homo()
        post_dat = orig.post_slope(min_val=2, max_val=10, start=coords[1], end=coords[0],
                                   cross=True)
        mean_valid = np.nanmean(np.hstack(data_valid[1][50]))
        mean_post = np.nanmean(np.hstack(post_dat[1][50]))
        assert all([ a==b for a, b in zip(post_dat[0], data_valid[0])])
    ...
    def test_post_elev(self, orig_homo):
        ...
        coords = pyosp.point_coords(start_end)
        orig = orig_homo()
        post_dat = orig.post_elev(min_val=5, max_val=20, start=coords[1], end=coords[0],
                                  cross=True)
        mean_valid = np.nanmean(np.hstack(data_valid[1][50]))
        mean_post = np.nanmean(np.hstack(post_dat[1][50]))
        assert all([ a==b for a, b in zip(post_dat[0], data_valid[0])])
